...
tapioca_image = pygame.image.load(
    os.path.join(images_folder, 'tapioca.png')).convert()
rock_image = pygame.image.load(
    os.path.join(images_folder, 'rock.png')).convert()

player_image = pygame.image.load(os.path.join(
    images_folder, 'Player(1).png')).convert()
PLAYER_WIDTH, PLAYER_HEIGHT = player_image.get_size()

lives_image = pygame.image.load(
    os.path.join(images_folder, 'lives.png')).convert()
lives_image = pygame.transform.scale(lives_image, (35, 35))
lives_image.set_colorkey((247, 247, 247))

boba_image = pygame.image.load(
    os.path.join(images_folder, 'boba.png')).convert()
boba_image.set_colorkey((247, 247, 247))

background_image = pygame.image.load(
    os.path.join(images_folder, 'background.png')).convert()
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rect = self.image.get_rect(bottomleft=(
            DISPLAY_WIDTH/2, DISPLAY_HEIGHT - PLAYER_HEIGHT))
...
class Tapioca(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect(
            center=(random.randint(0, DISPLAY_WIDTH), 0))
